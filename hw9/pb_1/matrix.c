 ...
 */
int getElement(matrix const *mtx, int row, int col, double *val);
 ...
 */
int sum(matrix const *mtx1, matrix const *mtx2, matrix *sum);
...
int dotProduct(matrix const *v1, matrix const *v2, double *prod);
...
void testBasic(void) {
  ...
  A = newMatrix(3, 3);
  assert(A);
  err = setElement(A, 1, 1, 1.0);
  assert(!err);
  err = setElement(A, 1, 2, .25);
  assert(!err);
  err = setElement(A, 1, 3, -.1);
  assert(!err);
  err = setElement(A, 2, 2, .4);
  assert(!err);
  err = setElement(A, 2, 3, .3);
  assert(!err);
  err = setElement(A, 3, 2, .1);
  assert(!err);
  err = setElement(A, 3, 3, -.3);
  assert(!err);
  printf("Matrix A:\n");
  err = printMatrix(A);
  assert(!err);
  ...
  B = newMatrix(3, 3);
  assert(B);
  err = setElement(B, 1, 1, .5);
  assert(!err);
  err = setElement(B, 2, 2, 2.0);
  assert(!err);
  err = setElement(B, 3, 3, 1.0);
  assert(!err);
  printf("\nMatrix B:\n");
  err = printMatrix(B);
  assert(!err);
  c = newMatrix(3, 1);
  assert(c);
  err = setElement(c, 1, 1, 1.0);
  assert(!err);
  err = setElement(c, 3, 1, 1.0);
  assert(!err);
  printf("\nVector c:\n");
  err = printMatrix(c);
  assert(!err);
  d = newMatrix(3, 1);
  assert(d);
  err = setElement(d, 2, 1, 1.0);
  assert(!err);
  err = setElement(d, 3, 1, 1.0);
  assert(!err);
  printf("\nVector d:\n");
  err = printMatrix(d);
  assert(!err);
  ...
  mdp = newMatrix(1, 1);
  assert(mdp);
  err = product(ct, d, mdp);
  assert(!err);
  err = getElement(mdp, 1, 1, &dp);
  assert(!err);
  printf("\nDot product (1): %.2f\n", dp);
  err = dotProduct(c, d, &dp);
  assert(!err);
  printf("\nDot product (2): %.2f\n", dp);
  err = product(A, c, d);
  assert(!err);
...
void testSymmetric() {
  ...
  err = setElement(A, 1, 3, -.1);
  assert(!err);
  err = setElement(A, 2, 1, .25);
  assert(!err);
  err = setElement(A, 2, 2, .4);
  assert(!err);
  err = setElement(A, 2, 3, .3);
  assert(!err);
  err = setElement(A, 3, 1, -.1);
  assert(!err);
  err = setElement(A, 3, 2, .3);
  assert(!err);
  err = setElement(A, 3, 3, -.3);
  assert(!err);
  printf("Matrix A:\n");
  err = printMatrix(A);
  assert(!err);
  printf("\nMatrix A is symmetric: %d\n", symmetric(A));
  deleteMatrix(A);
}

void testGetColumnGetRow() {
  matrix *A, *v;
  int err;
  A = newMatrix(3, 3);
  assert(A);
...
matrix *newMatrix(int rows, int cols) {
  ...
  /* set all data to 0 */
  for (i = 0; i < rows * cols; i++) m->data[i] = 0.0;
  return m;
}
...
int sum(matrix const *mtx1, matrix const *mtx2, matrix *sum) {
  ...
  if (mtx1->rows != mtx2->rows || mtx1->rows != sum->rows ||
      mtx1->cols != mtx2->cols || mtx1->cols != sum->cols)
    return -2;
...
int dotProduct(matrix const *v1, matrix const *v2, double *prod) {
  ...
  *prod = 0;
  for (i = 1; i <= v1->rows; i++) *prod += getE(v1, i, 1) * getE(v2, i, 1);
  return 0;
}

int isSquare(matrix const *mtx) { return mtx && mtx->rows == mtx->cols; }
...
int isDiagonal(matrix const *mtx) {
  ...
  for (col = 1; col <= mtx->cols; col++)
    for (row = 1; row <= mtx->rows; row++)
      if (row != col && getE(mtx, row, col) != 0.0) return 0;
  return 1;
}
...
int isUpperTriangular(matrix const *mtx) {
  ...
  for (col = 1; col <= mtx->cols; col++)
    for (row = col + 1; row <= mtx->rows; row++)
      if (getE(mtx, row, col) != 0.0) return 0;
  return 1;
}

int diagonal(matrix const *v, matrix *mtx) {
  int row, col;
  if (!v || !mtx || v->rows != mtx->rows || mtx->rows != mtx->cols) return -1;
  for (col = 1; col <= mtx->cols; col++)
    for (row = 1; row <= mtx->rows; row++)
      if (row == col)
...
matrix *getRow(matrix const *mtx, int row) {
  ...
  matrix *v;
  int col;
  v = newMatrix(1, mtx->cols);
  for (col = 1; col <= mtx->cols; col++) {
    setE(v, 1, col, getE(mtx, row, col));
  }
  return v;
}
